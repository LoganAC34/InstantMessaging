...
if debug:
    cfg_user_1 = config[header_debug][cfg_user_1_pc_variable]
    cfg_user_2 = config[header_debug][cfg_user_2_pc_variable]
...
class SendPanel(wx.Panel):
    ...
    def send_message(self, event):
        ...
        if msg:
            ...
            server.send_message(PC_Local_Name + u_separator + msg)
...
class MyFrame(wx.Frame):
    ...
    def OnSize(self, event):
        self.sub_panel_Log.Freeze()
        wx.CallAfter(self.sub_panel_Log.Thaw())
    ...
    @staticmethod
    def UpdateNow(downloaded_path, current_path, new_path, _pkl_sha, new_sha, _pkl_update):
        print("Running update file")
        sys.path.insert(0, tempfile.gettempdir())
        import Update
        Update.run(downloaded_path, current_path, new_path, _pkl_sha, new_sha, _pkl_update)
