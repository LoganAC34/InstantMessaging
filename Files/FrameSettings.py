...
from Files.wxglade import SettingsWindow
from Server import SocketWorkerThread as server
...
class FrameSettings(SettingsWindow):
    ...
    def Subtract_OnClick(self, event):
        ...
        while True:
            try:
                user = self.sizer_RemoteUsers.GetChildren()[n]  # n = user0 = User 1
                n += 1
                sizer = user.Sizer.StaticBox
                sizer.SetLabel(f'User {n}')
            except Exception as e:
                # print("No more users")
                # print(e)
                break
    ...
    def Apply_OnClick(self, event):  # wxGlade: SettingsWindow.<event_handler>
        ...
        while True:
            ...
            n += 1
            config.set_user_info('device_name', user_DeviceName, 'remote', n)
            config.set_user_info('alias', user_Alias, 'remote', n)
            config.set_user_info('override', user_Override, 'remote', n)
            print(f'REMOTE_USER_{n} | {user_DeviceName} | {user_Alias} | {str(user_Override)}')
